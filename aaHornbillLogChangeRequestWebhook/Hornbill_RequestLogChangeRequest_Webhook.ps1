##############################
#.SYNOPSIS
#Azure Powershell Runbook to log a new Change Request within Service Manager on a Hornbill Instance
#
#.DESCRIPTION
#Azure Powershell Runbook to log a new Change Request within Service Manager on a Hornbill Instance
...
#See the Hornbill_RequestLogChangeRequest_Workflow runbook for the available parameters.
#
#.NOTES
...
if ($null -ne $WebhookData) {
    ...
    if($requestObject.catalogName -and $requestObject.catalogName -ne ""){
        $catalogObj = Get-HB-CatalogID $requestObject.catalogName $requestObject.serviceName "Change Request"
    ...
    Add-HB-Param "description" $requestObject.description $false
    ...
    Add-HB-Param "changeType" $requestObject.changeType $false
